...
public class MantisModel<T extends MantisEntity> extends SinglePartEntityModel<T> {
    ...
    public void setAngles(MantisEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        ...
        // limbAngleScale (second last param in animateMovement) This scales how far into the animation cycle the model is considered to be, based on the limbSwing.
        // Higher values cause faster cycling through the animation pose frames, like speeding up the walking motion.

        // limbDistanceScale (last param) This scales how strong the animation appears, based on limbSwingAmount, which is usually proportional to movement intensity (e.g., running vs walking).
